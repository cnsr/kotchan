...
var get_country_by_ip = require('./utils/get-country-by-ip.js');
...
<<<<<<< HEAD
    socket.on('geoip_data', function(data) {
        ...
        get_country_by_ip(
            ...
            geoip_data, 
            function(){
                //console.log('emitting geoip_data for ' + socket.handshake.headers["x-real-ip"] + ": " + JSON.stringify(geoip_data));
                //Socket.io.sockets.in(data).emit('geoip_data', JSON.stringify(geoip_data));
                socket.emit('geoip_data', geoip_data);
            }
        );
    });
=======

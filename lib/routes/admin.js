...
router.post('/move', function (req, res, next) {
	try {
	    ...
	    check_password(password, function(err, matches) {
	        ...
	        send_data('int', 'delete', {count: chat_id}, 'count');
	        ...
	        return chat_db.find({count:chat_id},
	            function(e,d){
                    ...
                    if(d[0].is_convo_op) {
	                    chat_db.remove({convo: d[0].convo, is_convo_op: false}, function(e3) {
	                    	if (e3) {
	                    		console.log(e3);
	                    		return res.json({failure: e3.message});
	                    	}
                    	...
                    	send_data(data.chat, 'delete', data, config.board_fields);
                    	chat_db.update({count:chat_id}, data, function(e3) {
                                if (e3) {
                                    console.log(e3);
                                    return res.json({failure: e3.message});
                                }
	    ...
	    check_password(password, function(err, matches) {
	        ...
	        return chat_db.find({count:chat_id},
	            function(e,d){
                    ...
                    if(d[0].is_convo_op) {
	                    chat_db.remove({convo: d[0].convo}, function(e3) {
	                    	if (e3) {
	                    		console.log(e3);
	                    		return res.json({failure: e3.message});
	                    	}

	...
	try {
	    ...
	    check_password(password, function(err, matches) {
	        ...
	        chat_db.find({count: chat_id},
	            function(e,d){
	                if(d[0] && d[0].ip) {
	                	new ban_db({ip: d[0].ip, ban_date: new Date()}).save(function (e){
	                		if(e) {
	                			console.log(e);
							}
						});
	        ...
	        return chat_db.find({count:chat_id},
	            function(e,d){
	                ...
	                if(d[0] && d[0].image) {
                        ...
                        if(d[0].is_convo_op) {
	                    chat_db.update({convo: d[0].convo}, {'$set':{deleted:true}}, function(e3) {
	                    	if (e3) {
	                    		console.log(e3);
	                    		return res.json({failure: e3.message});
	                    	}
                            ...
                            add_to_chat(
                                {
                                    chat: d[0].chat,
                                    count: chat_id,
                                    /* body: "[color=red][b]user was gulaged for this post[/b][/color]", */ //workaround; commented by egor due to mongo's capped collections related shit "errmsg" : "Cannot change the size of a document in a capped collection"
                                    convo: d[0].convo,
                                    /* image_filename: "",
                                    image: "",
                                    thumb: "", */
                                ...
                                function(e2) {
                                    if (e2) {
                                        console.log("Error occured while add_to_chat: " + e2);
	    ...
	    check_password(password, function(err, matches) {
	        ...
	        return chat_db.find({count:chat_id},
	            function(e,d){
                    ...
                    chat_db.update({identifier: user_id}, {'$set': {deleted: true}}, function(e3) {
                        if (e3) {
                            console.log(e3);
                            return res.json({failure: e3.message});
                        }

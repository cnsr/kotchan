...
module.exports = function(req, res, callback) {
    ...
    ban_db.deleteOne,({ban_date:{'$lt':yesterday}}, function(e3){
    ...
    for(var j=0; j<config.banned_words.length; j++){
        if(req.body.body.toLowerCase().indexOf(config.banned_words[j])!==-1){
            return callback(new Error('ban_violation'));
        }
    }

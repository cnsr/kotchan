...
module.exports = function(data, callback) {
    ...
    try {
		...
		if (metadata.streams[0].codec_name.indexOf(format_names[extension]) == -1) {
		    ...
		    return callback(new Error('image file content does not match extension'));
                ...
                data.image_width = metadata.streams[0].width;
                data.image_height = metadata.streams[0].height;
		data.image_transparent = true;

                if (metadata.frames.length > 1) {
                    data.duration = 0;
                    metadata.frames.forEach(function(x) {
                        data.duration += parseFloat(x.pkt_duration_time);
                    });
                    //data.duration /= 100;
                }
                    ...
                    if (video_stream !== null && video_stream.width && video_stream.height) {
                        ...
                        if (video_stream.sample_aspect_ratio !== undefined) {
                            try {
                                ...
                                if(parseInt(parts[0])) data.image_width = Math.round(data.image_width * parseInt(parts[0]) / parseInt(parts[1]));
                            } catch(e) {

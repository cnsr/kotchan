...
var get_country_by_ip = require('./get-country-by-ip.js');
...
//var country_names = JSON.parse(fs.readFileSync('lib/utils/regioncodes.json'));

/*
function get_country_by_ip(user_ip, data, callback) {
...
function get_country(req, data, callback) {
    ...
    if (req.cookies.password_livechan ==  config.no_limit_cookie && req.body.country) {
       ...
       if (country_names[req.body.country.split('-')[0]]) {
		...
		data.country_name = (req.body.country!==undefined?req.body.country:default_country); // this is a temp fix until we get either a different country_names array or a bigger one
	}
        return callback();
    }

    get_country_by_ip(user_ip, data, callback);
...
/*
            ...
            // console.log('city.loopup: ' + data.country);
            return callback();
        } else {
            return callback();
            //return callback(new Error('proxy'));
